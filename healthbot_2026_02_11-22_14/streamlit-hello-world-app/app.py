...
def extract_patient_id(question: str) -> str | None:
    """
    Extracts patient ID from a question.
    Supports formats like:
      ...
      - p_386
      - patient P_21
    Returns normalized format: P_<number>
    """
    if not question:
        return None
    ...
    # Regex:
    # P optionally followed by _ or -
    # followed by digits
    pattern = re.compile(r"\bP[_\-]?\d+\b", re.IGNORECASE)
    ...
    # Normalize to P_<number>
    number = re.search(r"\d+", raw_id).group(0)
    normalized_id = f"P_{number}"

    return normalized_id

def retrieve_context(queryText: str):
    """Queries the Databricks Vector Search index for relevant documents."""
    query_vector = get_embeddings(queryText)
    ...
    pid = extract_patient_id(queryText)
    # st.write("pid....", pid)
    # st.write("query_text....", queryText)
    ...
    try:
        ...
        search_results = w.vector_search_indexes.query_index(
            ...
            query_text=pid if pid else queryText,
            query_type="FULL_TEXT" if pid else "ANN",
